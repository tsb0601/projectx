...
submit_script() {
    ...
    gcloud compute tpus tpu-vm ssh --zone "$ZONE" $TPU_NAME --project "$PROJECT" --worker=all \
        --command="cd ~/llava_base && git checkout $BRANCH && git pull && tmux send-keys -t $SESSION_NAME 'cd ~/llava_base && export WANDB_API_KEY=\"$WANDB_KEY\" && export WANDB_ENTITY=nyu-visionx && export WANDB_PROJECT=llava && bash $SCRIPT' C-m"
    if [ $? -ne 0 ]; then
