...
log "> ssl_eval/737k/2_stage_finetune.sh $@"
...
# if unfreezing the vision tower, ensure that the learning rate is set
if [ "$unfreeze_mm_vision_tower" == "True" ] && [ -z "$mm_vision_tower_lr" ]; then
    log "Unfreezing the vision tower but learning rate is not set. Exiting..."
    exit 1
fi

################# Check for previous completed runs #################

gcloud_path="gs://us-central2-storage/cambrian/checkpoints/ssl_exps/2stage_737k/$run_name"
