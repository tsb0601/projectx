...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"):
            ...
            unpad_merge = 'unpad' in getattr(config, 'mm_patch_merge_type', '')

        if unpad_merge or getattr(config, 'unpad', False):
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        image_features = self.get_model().get_vision_tower()(images)  # (batch_size, image_token_len, mm_hidden_size)
        image_features = self.get_model().mm_projector(image_features).to(images.dtype)  # (batch_size, image_token_len, hidden_size)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        """TODO: Need to understand why this change breaks GPU eval"""
