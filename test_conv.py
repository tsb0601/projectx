...
def train(INDEX, attn_implementation=None):
    ...
    global local_rank
    logger.info(f"Start training... Local rank: {local_rank} Index: {INDEX}")
    ...
    if 'mpt' in model_args.model_name_or_path:
        tokenizer = transformers.AutoTokenizer.from_pretrained(
            ...
            model_max_length=training_args.model_max_lengzth,
            padding_side="right"
        )
    else:
        tokenizer = transformers.AutoTokenizer.from_pretrained(
    ...
    log_rank0(f"At first is {conversation_lib.default_conversation}")
    ...
    log_rank0(f"AFTER CHANGE: Default conversation version: {conversation_lib.default_conversation.version}")

    log_rank0(f"Then it is {conversation_lib.default_conversation}")


    log_rank0("Configuring data module...")

    messages = [
        {"from": "human", "value": "What is your favourite condiment?"},
        {"from": "gpt", "value": "Well, I'm quite partial to a good squeeze of fresh lemon juice. It adds just the right amount of zesty flavour to whatever I'm cooking up in the kitchen!"},
        {"from": "human", "value": "Do you have mayonnaise recipes?"},
        {"from": "gpt", "value": "I sure do! Here's a simple recipe for homemade mayonnaise: Ingredients: 1 egg yolk, 1 teaspoon Dijon mustard, 1 cup vegetable oil, 1 tablespoon lemon juice, salt and pepper to taste. Instructions: 1. In a medium bowl, whisk together the egg yolk and Dijon mustard. 2. Slowly drizzle in the vegetable oil while whisking continuously until the mixture thickens. 3. Stir in the lemon juice, salt, and pepper. 4. Store in an airtight container in the refrigerator for up to one week. Enjoy!"},
        {"from": "human", "value": "Thank you! I'll try it out."},
        {"from": "gpt", "value": "You're welcome! Let me know if you have any other questions."},
        {"from": "human", "value": "I will. Goodbye!"},
        {"from": "gpt", "value": "Goodbye! Have a great day!"}
    ]
    ...
    # log_rank0(f"Data dict: {data_dict}")

    input_ids = data_dict["input_ids"]
    labels = data_dict["labels"]

    log_rank0(f"Input ids shape: {input_ids.shape}")
    log_rank0(f"Input ids: {input_ids}")

    # log_rank0(f"Labels shape: {labels.shape}")
    # log_rank0(f"Labels: {labels}")

    # decode
    input_ids_decoded = tokenizer.decode(input_ids[0])
    log_rank0(f"Input ids decoded:\n{input_ids_decoded}")
    # labels_decoded = tokenizer.decode(labels[0])
    # log_rank0(f"Labels decoded:\n{labels_decoded}")

    # log_rank0(f"diff:  {input_ids[0] - labels[0]}")
